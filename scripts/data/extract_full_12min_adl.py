...
Outputs: 
- `adl_fills_full_12min_raw.csv` → consumed by `scripts/analysis/adl_net_volume.py`
...
def extract_adl_fills():
    ...
    print("=" * 80)
    print("EXTRACTING FULL 12-MINUTE ADL DATA FROM S3")
    print("=" * 80)
    ...
    with lz4.frame.open(S3_DATA_FILE, 'rb') as f:
        for line_num, line in enumerate(f, 1):
            ...
            try:
                ...
                # Get block time (it's a string in ISO format with nanoseconds)
                block_time_str = data.get('block_time', '')
                if not block_time_str:
                    continue
                
                # Parse ISO timestamp - truncate nanoseconds to microseconds (Python limit)
                # Format: 2025-10-10T21:15:00.043230479 (9 decimal places)
                # Python supports up to 6 decimal places (microseconds)
                if '.' in block_time_str:
                    base, frac = block_time_str.rsplit('.', 1)
                    # Take only first 6 digits of fractional seconds
                ...
                # Process events (array of [user, fill_data] pairs)
...
def main():
    ...
    # Print comparison
    print_comparison(285_546_805, total_notional)  # Compare with 2-min sample
