...
- Replayed **3,239,706 on-chain events** (fills, funding, ledger) from the clearinghouse snapshot at 20:04:54 UTC through 21:27:00 UTC using `scripts/reconstruction/full_analysis_realtime.py`.
- Produced the **canonical 34,983-event dataset** (`data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv`) with real-time account value, leverage, equity, and negative-equity detection (fixed position size calculation, December 2025).
...
- Cross-validated downstream analyses (`docs/findings/ADL_PRIORITIZATION_VERIFIED.md`, `docs/findings/ADL_PRIORITIZATION_ANALYSIS_LOCAL.md`, `docs/findings/INSURANCE_FUND_IMPACT.md`) on the regenerated dataset.
...
| Dataset location | `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv` |

The replay is reproducible by running `python scripts/reconstruction/full_analysis_realtime.py` from the repository root. A step-by-step walkthrough lives in `docs/analysis/REAL_TIME_RECONSTRUCTION_SUMMARY.md`.
...
| File | Purpose |
|------|---------|
| `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv` | Canonical per-position dataset (34,983 rows) with realtime account values, leverage, equity, negative-equity flag, and ADL counterparties (fixed position size calculation, December 2025). |
...
| `scripts/reconstruction/full_analysis_realtime.py` | Replay script (clearinghouse snapshot ➝ realtime metrics). |
| `docs/analysis/REAL_TIME_RECONSTRUCTION_SUMMARY.md` | Detailed methodology and reproducibility notes. |
| `docs/findings/ADL_PRIORITIZATION_VERIFIED.md` | Public-facing verification that ADL targets profit, refreshed against the canonical replay. |
...
- ✅ `is_negative_equity` flag aligns with `total_equity < 0` and feeds the insurance-fund impact analysis (`docs/findings/INSURANCE_FUND_IMPACT.md`).
...
- ✅ ADL prioritization timing tests rerun on the canonical dataset (see `docs/findings/ADL_PRIORITIZATION_ANALYSIS_LOCAL.md`).
- ✅ Insurance-fund impact quantified: −$23,191,104 (see `docs/findings/INSURANCE_FUND_IMPACT.md`).
...
- Reproducibility: `scripts/reconstruction/full_analysis_realtime.py`, `docs/analysis/REAL_TIME_RECONSTRUCTION_SUMMARY.md`
- Downstream analyses refreshed against the canonical data:
  - `docs/findings/ADL_PRIORITIZATION_VERIFIED.md`
  - `docs/findings/ADL_PRIORITIZATION_ANALYSIS_LOCAL.md`
