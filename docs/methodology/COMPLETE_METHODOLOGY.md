...
**Script**: `scripts/data/extract_full_12min_adl.py`
...
**Script**: `scripts/data/analyze_clearinghouse.py`
...
- `docs/reports/FINDINGS_VERIFICATION_REPORT.md` – regression tests that rerun prioritization, isolation, timing, and insurance analyses on the canonical dataset.
---
...
**Documents**:
- `data/canonical/cash-only balances ADL event orderbook 2025-10-10/ADL_NET_VOLUME_FULL_12MIN.md` (or regenerate with `scripts/analysis/adl_net_volume.py`)
...
**Documents**:
- `docs/findings/PER_ASSET_ISOLATION.md`
...
**Documents**:
- `docs/findings/ADL_PRIORITIZATION_VERIFIED.md`
...
**Documents**:
- `docs/findings/INSURANCE_FUND_IMPACT.md`
- `scripts/reconstruction/full_analysis_realtime.py` (methodology)
...
- `docs/reports/FINDINGS_VERIFICATION_REPORT.md` - Automated verification results
...
```python
import pandas as pd
adl = pd.read_csv('data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv')
...
Run `python3 ../"ADL Net Volume"/scripts/verification/verify_all_findings.py` to regenerate the verification report covering prioritization, isolation, timing, and insurance analyses.

### Validation Checklist

- [ ] S3 data downloaded and decompressed (`20.lz4`, `21.lz4`)
- [ ] Event counts match (63,637 liquidations, 34,983 ADL)
- [ ] `scripts/reconstruction/full_analysis_realtime.py` replay completes (3,239,706 events processed)
...
- [ ] `scripts/verification/verify_all_findings.py` passes all prioritization/isolation/timing/insurance checks
...
| Account value / equity | 100% | `scripts/reconstruction/full_analysis_realtime.py` replay
...
├── data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_fills_full_12min_raw.csv          # All 34,983 ADL events
├── data/canonical/cash-only balances ADL event orderbook 2025-10-10/liquidations_full_12min.csv            # All 63,637 liquidation events
├── adl_net_volume_full_12min.csv         # Aggregated by ticker (162 tickers)
├── combined_impact_by_ticker.csv         # Liquidations + ADL by ticker
└── data/canonical/cash-only balances ADL event orderbook 2025-10-10/ADL_NET_VOLUME_FULL_12MIN.md          # Full report
```
...
├── docs/analysis/CLEARINGHOUSE_ANALYSIS_SUMMARY.md      # Phase 3 methodology update
...
├── docs/findings/ADL_PRIORITIZATION_VERIFIED.md         # ⭐ ADL targets profit
├── docs/findings/ADL_MECHANISM_RESEARCH.md              # Counterparty mechanics
├── docs/analysis/CASCADE_TIMING_ANALYSIS.md             # ~64-second delay
├── docs/analysis/BATCH_PROCESSING_DISCOVERY.md          # Sequential execution
├── docs/findings/PER_ASSET_ISOLATION.md                 # Zero cross-asset ADL
└── TOTAL_IMPACT_ANALYSIS.md               # Combined stats (regenerate with `scripts/analysis/total_impact_analysis.py`)
```

**Processing Scripts**:
```
/Users/thebunnymac/Desktop/ADL Net Volume/
├── scripts/data/extract_full_12min_adl.py              # S3 event extraction
├── prepare_data.py                        # Visualization dataset builder (canonical files)
└── scripts/analysis/total_impact_analysis.py              # Aggregate statistics

/Users/thebunnymac/Desktop/ADL Clearinghouse Data/
├── scripts/data/analyze_clearinghouse.py               # Snapshot bootstrap utilities
└── scripts/reconstruction/full_analysis_realtime.py              # Canonical real-time replay pipeline
```
