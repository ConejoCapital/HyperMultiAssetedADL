...
> Associated script: `scripts/analysis/batch_processing_analysis.py`
...
Using the regenerated realtime files (`data/canonical/cash-only balances ADL event orderbook 2025-10-10/liquidations_full_12min.csv`, `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_fills_full_12min_raw.csv`) we reran `python prepare_data.py` (the same pipeline that powers `public/events.json` in HyperFireworks). A follow-up check over all 98,620 events confirmed:
