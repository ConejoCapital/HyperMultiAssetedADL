...
> Associated script: `scripts/analysis/adl_mechanism_analysis.py`
...
This report has been refreshed using the canonical real-time reconstruction dataset (`data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv`, generated December 2025 with fixed position size calculation).
...
- **Data sources**: `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv`, `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_fills_full_12min_raw.csv`, `data/canonical/cash-only balances ADL event orderbook 2025-10-10/liquidations_full_12min.csv`
...
- **Insurance fund absorption**: Real-time reconstruction identified **302 negative-equity accounts** and **$23,191,104** of residual loss absorbed by the insurance/HLP buffers (`docs/findings/INSURANCE_FUND_IMPACT.md`).
...
1. **ADL Prioritization**: How does Hyperliquid choose which profitable traders to ADL?
   - ✅ **ANSWERED** with the realtime dataset (`data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv`)
   - **ADL targets PROFIT, not leverage**
   - 94.5% of ADL'd positions were profitable (avg +80.6% PNL)
   - Median leverage was only 0.18x (LOW!)
   - See `docs/findings/ADL_PRIORITIZATION_VERIFIED.md` and `docs/reports/FINDINGS_VERIFICATION_REPORT.md`

2. **Partial ADL Coverage**: Why was only 85% of the $204.7M liquidation stack covered by this single ETH ADL?
   - ✅ **EXPLAINED**: Canonical realtime data shows 34,983 ADLs across the burst. The remaining ~$30.5M was satisfied by follow-on ADLs in the same millisecond cluster **plus** $23,191,104 in insurance-fund absorption (quantified in `docs/findings/INSURANCE_FUND_IMPACT.md`).
...
3. **Cascade Effect**: How do liquidations trigger more liquidations?
   ...
   - See `docs/analysis/CASCADE_TIMING_ANALYSIS.md`, `docs/analysis/BATCH_PROCESSING_DISCOVERY.md`, and run `scripts/analysis/total_impact_analysis.py` for total impact analysis.
...
4. **Insurance Fund Impact**: What's the size of the gap the fund had to absorb?
   ...
   - Details in `docs/findings/INSURANCE_FUND_IMPACT.md`, `docs/analysis/REAL_TIME_RECONSTRUCTION_SUMMARY.md`, and `docs/reports/LEVERAGE_CORRECTION.md`.
...
- **Repository files**:
  - `data/canonical/cash-only balances ADL event orderbook 2025-10-10/adl_detailed_analysis_REALTIME.csv` – Canonical per-position dataset (fixed position size calculation)
  - `adl_by_user_REALTIME.csv` – User-level aggregations
  - `adl_by_coin_REALTIME.csv` – Asset-level aggregations
  - `docs/analysis/REAL_TIME_RECONSTRUCTION_SUMMARY.md` – Full methodology
